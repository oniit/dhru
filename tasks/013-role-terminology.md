# Task 013: Penyesuaian Terminologi Role Lokal (Pravesi, Sisya, Charya, Publik)
...
2. **`bot/handlers/commands.py` (Command `/all` / `/tagall`)**:
   - Menambahkan opsi parameter filter langsung seperti: `/tagall sisya`, `/tagall pravesi`, `/tagall charya`, `/tagall publik`.
...
3. **`bot/handlers/commands.py` (Command `/daftar`)**:
   ...
   - Menggantinya dengan `/daftar sisya`, `/daftar charya`, `/daftar pravesi`, `/daftar publik` yang otomatis mengelompokkan data secara rapi.
4. **`bot/handlers/commands.py` (Command `/agra`)**:
   - Memperbarui papan peringkat `/agra top` agar nama tampilannya berubah menjadi _(Pravesi)_, _(Sisya)_, _(Charya)_, dan _(Publik)_.
