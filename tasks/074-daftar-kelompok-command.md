...
- Di `bot/handlers/commands.py` pada fungsi `cmd_daftar`:
  ...
  - Jika cocok, sistem akan mengambil nilai ID aslinya, lalu mengiterasi seluruh pengguna dengan role `maba` yang nilai `maba_group`-nya sesuai dengan ID tersebut.
  - Opsi perintah ini (`/daftar kelompok <nama_kelompok>`) telah disisipkan juga ke dalam pesan *help* balasan bawaan.
